...
public class BackgammonBoardGenerator : MonoBehaviour
{
    ...
    public float pointWidth = 0f;      
    public float pointLength = 4f;
    public float baseThickness = 0.1f;
    public float yOffset = 0.003f;
    public Color topA = new(0.85f,0.85f,0.85f), topB = new(0.2f,0.2f,0.2f);
    public Color botA = new(0.85f,0.85f,0.85f), botB = new(0.2f,0.2f,0.2f);
    ...
    void Build()
    {
        ...
        br.sharedMaterial = new Material(Shader.Find("Universal Render Pipeline/Lit")){ color = baseColor };
    ...
    void CreateTriangle(Vector3 localCenter, Vector3 dir, float width, float length, Color col, string name)
    {
        ...
        var mat = new Material(Shader.Find("Universal Render Pipeline/Lit")){ color = col };
        go.GetComponent<MeshRenderer>().sharedMaterial = mat;

        float hw = width*0.5f;
        var tip = dir.normalized * length;
